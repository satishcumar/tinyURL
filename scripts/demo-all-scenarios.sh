...
run_scenario() {
  ...
  execution_id="$(python3 -c 'import json,sys; print(json.load(sys.stdin)["id"])' <<<"${created}")"
  ...
  if [[ "${schema_approval}" == "true" ]]; then
    python3 -c 'import json,sys; assert json.load(sys.stdin)["status"] == "AWAITING_SCHEMA_APPROVAL"' <<<"${approved}"
  ...
  python3 -c 'import json,sys; value=json.load(sys.stdin); assert value["status"] == "COMPLETED"; assert value["metrics"]["successRate"] == 1.0' <<<"${completed}"
