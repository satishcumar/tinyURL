...
execution_id="$(py -c 'import json,sys; print(json.load(sys.stdin)["id"])' \
  <<<"${requirement_response}")"

echo "Created workflow: ${execution_id}"
py -m json.tool <<<"${requirement_response}"
...
echo "Approved plan"
py -m json.tool <<<"${approval_response}"
...
py -m json.tool <<<"${execution_response}"

echo "Reviewable artifacts"
curl --fail --silent --show-error \
  "${api_base}/api/v1/workflows/${execution_id}/artifacts" | py -m json.tool
